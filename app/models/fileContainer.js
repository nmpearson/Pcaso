...
FileContainerSchema.statics.convertDisplaySettingsToLegacy = function (
  displaySettings
) {
  var legacySettings = {
    "fields-pca": [],
...
FileContainerSchema.statics.register = function (
  parent,
  file,
  settings,
  callback
  ...
  if (!parent.links || !parent.links.link || !parent.links.local) {
    console.warn(
      "Warning: parent.links is missing or incomplete. Setting default links."
    );
    parent.links = {
  ...
  var fileContainer = new this({
    ...
    links: {
      thumbnail:
        parent.publicDataPath + "/files/thumbnails/" + documentId + ".png",
  ...
  if (fileContainer.isNew) {
    ...
    conn.once("open", function () {
      ...
      writestream.on("finish", function () {
        if (!fileContainer.fileOptions.keepFile)
          fs.unlinkSync(fileContainer.file.path);

        // create thumbnail path and thumbnail
        mkdirp(
          fileContainer.localDataPath + "/files/thumbnails/",
          function (mkdirErr) {
            if (mkdirErr) return next(mkdirErr);

            generateThumbnail(
              fileContainer.localDataPath +
                "/files/thumbnails/" +
                fileContainer._id +
                ".png",
              next
...
FileContainerSchema.methods.viewableTo = function (user) {
  ...
  return (
    this.displaySettings.visibility === "PUBLIC" ||
